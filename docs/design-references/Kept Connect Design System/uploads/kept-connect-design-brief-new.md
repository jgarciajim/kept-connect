...
1. **Reassuring.** A stranger is coming to a home. Every surface should quietly signal *this person is vetted, this is handled, your money is safe.* Trust is the product (§2.2).
1. **Warm, but disciplined.** Connect carries brand warmth the ops tool won’t — but only in the human moments, never on the data (§1.2).
...
1. **Post-a-job composer** (§3.1) — prove the warm-moment system.
...
1. **The provider app** (§4) — the cool sibling; can run in parallel.
1. **Marketing site** — the warmest expression; where the blue-accent decision lands, if at all.
